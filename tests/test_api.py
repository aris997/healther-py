...
async def test_register_login_and_workspace_flow(app):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post("/api/auth/register", json={"email": "alice@example.com", "password": "secret123"})
        assert resp.status_code == 201, resp.text
        user_id = resp.json()["id"]

        token_resp = await client.post("/api/auth/token", json={"username": "alice@example.com", "password": "secret123"})
        ...
        ws_resp = await client.post("/api/workspaces", json={"name": "Team A", "is_public": True}, headers=headers)
